...
def _plot_skel_fit(target_df, fit_rows, before_xyz, after_xyz, all_after=None, all_names=None):
    try:
        ...
        fig.update_layout(height=620,margin=dict(l=0,r=0,t=45,b=0),title="V110.2.3 · Frame semilla · XYZ objetivo vs SKEL",scene=dict(aspectmode="data"))
        st.plotly_chart(fig,use_container_width=True)
    except Exception as exc:
        st.caption(f"Visualización del fit no disponible: {exc}")

# V110.2.3 · HIERARCHICAL RETARGETING
...
# V110.2.3 · propagación temporal jerárquica con 16 DOF activos.
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None):
    ...
    return {'version':'110.2.3','retargeting_mode':'hierarchical_16dof','scale':float(fixed_scale.cpu()),
...
def _plot_skel_sequence_animation(seq):
    ...
    try:
        ...
        fig.update_layout(
            height=680,margin=dict(l=0,r=0,t=45,b=0),title="V110.2.3 · SKEL · marcha propagada temporalmente",
...
def _plot_simplified_xyz_animation(motion):
    ...
    try:
        ...
        fig.update_layout(height=600,margin=dict(l=0,r=0,t=45,b=0),title='V110.2.3 · Vídeo 3D simplificado V104/V107 · no calibrado',
...
# V110.2.3 · SKEL MESH WALKER: malla corporal real `skin_verts` sobre la secuencia temporal validada.
...
def _mesh_sequence_npz_bytes(mesh_seq):
    buf=io.BytesIO()
    payload={
        "version":np.asarray([str(mesh_seq.get("version","110.2.2"))]),
...
def _plot_skel_mesh_walker(mesh_seq):
    ...
    try:
        ...
        fig.update_layout(
            height=760,margin=dict(l=0,r=0,t=50,b=0),title="V110.2.3 · SKEL MESH WALKER · marcha anatómica 3D",
            scene=dict(aspectmode="data",xaxis=dict(range=[mins[0]-pad[0],maxs[0]+pad[0]]),yaxis=dict(range=[mins[1]-pad[1],maxs[1]+pad[1]]),zaxis=dict(range=[mins[2]-pad[2],maxs[2]+pad[2]])),
...
def render_skel_poc_panel(motion):
    ...
    st.markdown("**Modelo SKEL privado · runtime V110.2.3 CPU + Hierarchical Retargeting + Mesh Walker**")
    ...
    if not runtime:
        st.error("El bundle privado es válido, pero el runtime SKEL CPU no ha podido prepararse. V110.2.3 evita deliberadamente moderngl-window para mantener NumPy 2.x compatible con Pose2Sim/OpenSim.")
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                st.info("Puerta de runtime superada. V110.2.3 valida primero el frame semilla y habilita después la propagación temporal hacia toda la secuencia.")

                st.markdown("### V110.2.3 · Frame semilla SKEL ↔ XYZ")
                ...
                try:
                    ...
                    export={
                        "version":"110.2.2","frame":int(best_i)+1,"gender":gender,"rmse_before":fit["rmse_before"],"rmse_after":fit["rmse_after"],
                    ...
                    st.download_button("⬇️ Descargar ajuste SKEL frame 1 (JSON)",json.dumps(export,ensure_ascii=False,indent=2).encode("utf-8"),"V110_2_3_SKEL_fit_frame1.json","application/json",use_container_width=True)

                    st.markdown("### V110.2.3 · Propagación temporal 1→75")
                    st.caption("La escala corporal y betas quedan congeladas. V110.2.3 añade regularización anatómica en joints no observados, límites suaves de pose y continuidad temporal para impedir soluciones de bajo RMSE pero geométricamente deformadas.")
                    if st.button("▶️ Procesar secuencia completa y generar marcha SKEL",type="primary",use_container_width=True,key="v110_2_3_run_sequence"):
                    ...
                    if isinstance(seq,dict) and seq.get("frames"):
                        ...
                        if unsafe:
                            ...
                            st.success("✅ Control anatómico superado en todos los frames resueltos: sin plegados extremos detectados por la auditoría V110.2.3.")
                        ...
                        st.download_button("⬇️ Descargar secuencia SKEL V110.2.3 (JSON)",json.dumps(export_seq,ensure_ascii=False).encode("utf-8"),"V110_2_3_SKEL_sequence.json","application/json",use_container_width=True)
                        ...
                        st.markdown("### V110.2.3 · SKEL MESH WALKER")
                        st.caption("Convierte exclusivamente la secuencia V110.2.3 activa en `skin_verts`: no reutiliza estados de V110.1.9/V110.2.1. Betas=0 y la escala se copia y verifica contra la secuencia temporal actual antes de guardar el NPZ.")
                        if st.button("🧍▶️ Generar malla corporal SKEL y reproducir marcha",type="primary",use_container_width=True,key="v110_2_3_build_skin_mesh"):
                            ...
                            try:
                                ...
                                st.session_state["v110_2_3_mesh_sequence"]=mesh_seq
                        ...
                        mesh_seq=st.session_state.get("v110_2_3_mesh_sequence")
                        if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                            ...
                            st.download_button("⬇️ Descargar SKEL Mesh Walker V110.2.3 (NPZ)",_mesh_sequence_npz_bytes(mesh_seq),"V110_2_3_SKEL_mesh_sequence.npz","application/octet-stream",use_container_width=True)
                        ...
                        st.info("El frame semilla está validado. Pulsa el botón anterior para resolver frames 2→75; después V110.2.3 habilitará la malla corporal animada.")
                except Exception as fit_exc:
                    st.error("SKEL funciona, pero V110.2.3 no ha podido completar el ajuste del frame semilla.")
